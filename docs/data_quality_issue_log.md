...
## Coverage Note
This log catalogues issues actually found during Bronze-level QA profiling. 13 of the project's **19 delivered tables** have at least one logged issue above, plus 2 issues (DQ-018, DQ-019) on the Silver-only reference tables `simulated_nhis_healthcare_claims` and `llm_finetune` — kept here for historical completeness even though those two tables (and the third reference-only table, `train_test_split`) are **not** counted among the 19. The remaining delivered tables with no logged issue — `allergies`, `devices`, `encounters`, `immunizations`, `payer_rules`, and `supplies` — were profiled with the same QA process ([`data_quality-rules.md`](./data_quality-rules.md)) and no material issues were found worth logging; their absence here reflects a clean QA pass, not a skipped check.
