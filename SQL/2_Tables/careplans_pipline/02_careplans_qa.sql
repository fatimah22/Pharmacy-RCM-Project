...
SELECT
    ...
    COUNT(DISTINCT LOWER(TRIM(place_of_service_desc))) AS description_count
FROM bronze.claims_main
GROUP BY Place_of_Service_Code
HAVING COUNT(DISTINCT LOWER(TRIM(place_of_service_desc))) > 1;

-- Reverse check for place of service mapping
SELECT
    LOWER(TRIM(place_of_service_desc)) AS normalized_description,
    COUNT(DISTINCT Place_of_Service_Code) AS code_count
FROM bronze.claims_main
GROUP BY LOWER(TRIM(place_of_service_desc))
